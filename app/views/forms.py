...
class LoginForm(Form):
    ...
    remember_me=BooleanField(u"记住登陆状态")
...
class RegisterForm(Form):
    ...
    password = PasswordField(u"密码：", validators=[DataRequired(u"不能为空"), EqualTo(u"password2", u"密码必须一致"),                                                 Length(6, 16, u"密码长度必须6~16位")])  # ,"密码格式错误"
    password2 = PasswordField(u"再次输入密码：", validators=[DataRequired(u"不能为空")])  # ,"密码格式错误"
    submit = SubmitField(u"注册")

class EditPostForm(Form):
    title=StringField(u"*标题", validators=[Required(u"不能为空")])
    # content=StringField(u"dff",validators=[Required(u"不能为空")])
    content=TextAreaField(label="",validators=[Required(u"不能为空")])
